...
let iter t ~(f : Pixel.t -> unit) = Array.iter t.image ~f

let iter2 t img2 ~(f : Pixel.t -> Pixel.t -> unit) =
  Array.iter2_exn t.image img2.image ~f
;;

let for_all2 t img2 ~(f : Pixel.t -> Pixel.t -> bool) =
  Array.for_all2_exn t.image img2.image ~f
;;
